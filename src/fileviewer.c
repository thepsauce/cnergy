...
files_compare(const void *a, const void *b, void *arg)
{
	...
	if (!cmp)
		switch (flags.sort)
		{
		case FILEVIEW_SORT_ALPHA:
			cmp = strcasecmp(fc1->name, fc2->name);
			break;
		case FILEVIEW_SORT_MODTIME:
			cmp = fc1->mtime - fc2->mtime;
			break;
		case FILEVIEW_SORT_CHGTIME:
			cmp = fc1->ctime - fc2->ctime;
			break;
		}
...
int fileviewer_render(struct window *win)
{
	...
	while (1)
	{
		...
		{
			if (win->selected == y)
				attrset(type == FC_TYPE_DIR ? dirselectedColor : type == FC_TYPE_REG ? fileselectedColor
																					 : execselectedColor);
			else
				attrset(type == FC_TYPE_DIR ? dirColor : type == FC_TYPE_REG ? fileColor
																			 : execColor);
...
bool fileviewer_bindcall(struct window *win, struct binding_call *bc, ptrdiff_t param, ptrdiff_t *pCached)
{
	...
	else
	{
		...
		case BIND_CALL_TOGGLEHIDDEN:
			win->flags.hidden = !win->flags.hidden;
			break;
		case BIND_CALL_TOGGLESORTTYPE:
			win->flags.sortType = !win->flags.sortType;
			break;
		case BIND_CALL_TOGGLESORTREVERSE:
			win->flags.sortReverse = !win->flags.sortReverse;
			break;
		case BIND_CALL_SORTALPHABETICAL:
			win->flags.sort = FILEVIEW_SORT_ALPHA;
			break;
		case BIND_CALL_SORTMODIFICATIONTIME:
			win->flags.sort = FILEVIEW_SORT_MODTIME;
			break;
		case BIND_CALL_SORTCHANGETIME:
			win->flags.sort = FILEVIEW_SORT_CHGTIME;
			break;
		default:
			return false;
		}
