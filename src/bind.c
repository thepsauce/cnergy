...
void print_binding_calls(struct binding_call *calls, unsigned nCalls)
{
	printf("{ ");
...
void print_modes(struct binding_mode *m)
{
	if (!m)
		return;
...
int bind_exec(const int *keys, ptrdiff_t amount)
{
	struct binding *bind;
	...
	for (bcs = bind->calls, nbc = bind->nCalls; nbc; nbc--, bcs++)
	{
		...
		case BIND_CALL_MOVEWINDOW_BELOW:
		{
			...
			struct window *(*const next)(const struct window *) =
				bc.type == BIND_CALL_MOVEWINDOW_RIGHT ? (m > 0 ? window_right : window_left) : m > 0 ? window_below
																									 : window_above;
		...
		while (((bc.flags & FBIND_CALL_OR) && !(frame.state |= b)) || ((bc.flags & FBIND_CALL_AND) && !(frame.state &= b)))
		{
		end_frame:
			do
			{
