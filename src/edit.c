...
bool edit_bindcall(struct window *win, struct binding_call *bc, ptrdiff_t param, ptrdiff_t *pCached)
{
	...
	case BIND_CALL_MOVECURSOR:
		return buffer_movecursor(buf, param) == param;
	case BIND_CALL_MOVEHORZ:
		return buffer_movehorz(buf, param) == param;
	...
	case BIND_CALL_MOVEVERT:
		return buffer_movevert(buf, param) == param;
	case BIND_CALL_INSERT:
		buffer_insert(buf, bc->str, strlen(bc->str));
		return true;
	case BIND_CALL_DELETE:
		return buffer_delete(buf, param) == param;
	case BIND_CALL_DELETELINE:
		return buffer_deleteline(buf, param) == param;
